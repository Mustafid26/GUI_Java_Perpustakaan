...
public class BukuController implements Initializable {
    ObservableList<Buku> listBuku = FXCollections.observableArrayList() ;boolean flagAdd = true;
    @FXML
    ...
    void setFilter(){
        ...
        tfsearch.textProperty().addListener((observable,oldValue,newValue)->{
            filterData.setPredicate(Buku->{
                ...
                ( String.valueOf(Buku.getIdbuku()).toLowerCase().indexOf(searchKeyword) >-1){
                    return true;
                }
                else
                    return false;
        ...
        sortedData.comparatorProperty().bind(tvbuku.comparatorProperty());tvbuku.setItems(sortedData);
    }
    void initTabel(){
        cidbuku.setCellValueFactory(new
                PropertyValueFactory<Buku,Integer>("idbuku"));
        cjudul.setCellValueFactory(new
                PropertyValueFactory<Buku,String>("judul"));
        cpenerbit.setCellValueFactory(new
                PropertyValueFactory<Buku,String>("penerbit"));
        cpenulis.setCellValueFactory(new
                PropertyValueFactory<Buku,String>("penulis"));
        ctahun_terbit.setCellValueFactory(new
                PropertyValueFactory<Buku,Integer>("tahun_terbit"));
    }
    void loadData(){
